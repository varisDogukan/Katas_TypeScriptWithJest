...
Task
Given the a list of unique numbers sorted in ascending order, return a 
new list so that the values increment by 1 for each index from the minimum
...
export default function pipeFix(numbers: number[]): number[] {
  const result: number[] = [];
